...
namespace DeePID
{
  ...
  class DeePIDTreeMaker : public art::EDAnalyzer
  {
    public:
    
      DeePIDTreeMaker(fhicl::ParameterSet const& pset);
...
DeePID::DeePIDTreeMaker::DeePIDTreeMaker(fhicl::ParameterSet const& pset):
  EDAnalyzer(pset)
{
} // DeepIDTreeMaker::DeepIDTreeMaker(fhicl::ParameterSet const& pset)

//---------------------------------------------------------------------------------------------------------

void DeePID::DeePIDTreeMaker::beginJob()
{
  art::ServiceHandle<art::TFileService> tfs;
  DeePIDtree = tfs->make<TTree>("DeePIDtree","DeePIDtree");
  
  DeePIDtree->Branch( "initEnergy", &initialE );
  DeePIDtree->Branch( "pdgCode", &pdgCode );
  
  DeePIDtree->Branch( "wireList",     &wireList     );
  DeePIDtree->Branch( "planeList",    &planeList    );
  DeePIDtree->Branch( "TPCList",      &TPCList      );
  DeePIDtree->Branch( "integralList", &integralList );
  DeePIDtree->Branch( "timeList",     &timeList     );

  DeePIDtree->Branch( "wireListCheat",     &wireListCheat     );
  DeePIDtree->Branch( "planeListCheat",    &planeListCheat    );
  DeePIDtree->Branch( "TPCListCheat",      &TPCListCheat      );
  DeePIDtree->Branch( "integralListCheat", &integralListCheat );
  DeePIDtree->Branch( "timeListCheat",     &timeListCheat     );

}

//---------------------------------------------------------------------------------------------------------

void DeePID::DeePIDTreeMaker::analyze(art::Event const & evt)
{
  // ==================================================================
  // Data labels
  // ==================================================================
  ...
  std::string pandoraCheatLabel  = "cheatPandora";
...
//    TPCList.push_back(      tpc      );
//    integralList.push_back( integral );
//    timeList.push_back(     peakTime );
//
//  }
...
std::vector< int > DeePID::DeePIDTreeMaker::WireCorrector ( int wire, int plane, int tpc )
{
...
void DeePID::DeePIDTreeMaker::PFinfo ( art::FindManyP<recob::Cluster>             const & PFtoClust,
                                       art::FindManyP<recob::Cluster>             const & HitToClust,
                                       std::vector< art::Ptr<recob::Hit> >        const & hitList,
                                       std::vector< art::Ptr<recob::PFParticle> > const & PFPartList,
                                       bool   const & cheat )
...
void DeePID::DeePIDTreeMaker::DaughterInfo ( std::vector< art::Ptr<recob::PFParticle> > const & PFPartList,
                                             art::FindManyP<recob::Cluster>             const & PFtoClust,
                                             std::map< art::Ptr<recob::Cluster>, std::vector< art::Ptr<recob::Hit> > > const & clustersToHits,
                                             std::vector<size_t> const & daughters,
                                             std::vector< int >    & wires,
...
DEFINE_ART_MODULE(DeePID::DeePIDTreeMaker)
